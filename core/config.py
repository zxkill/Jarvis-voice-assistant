...
class LlmConfig:
    ...
    xiaozhi_client_id: str
    xiaozhi_manager_url: str
    xiaozhi_manager_secret: str
...
def load_config(path: str = "config.ini") -> AppConfig:
    ...
    if "LLM" not in parser:
        ...
        parser.set(
            "LLM",
            "xiaozhi_client_id",
            os.getenv("XIAOZHI_CLIENT_ID", os.getenv("XIAOZHI_DEVICE_ID", "jarvis-client")),
        )
        parser.set(
            "LLM",
            "xiaozhi_manager_url",
            os.getenv("XIAOZHI_MANAGER_URL", "https://api.tenclass.net/xiaozhi"),
        )
        parser.set("LLM", "xiaozhi_manager_secret", os.getenv("XIAOZHI_MANAGER_SECRET", ""))
    ...
    llm = LlmConfig(
        ...
        xiaozhi_client_id=_env_or_cfg(
            parser,
            "LLM",
            "xiaozhi_client_id",
            "XIAOZHI_CLIENT_ID",
            default=parser.get("LLM", "xiaozhi_device_id", fallback="jarvis-client"),
        ),
        xiaozhi_manager_url=parser.get("LLM", "xiaozhi_manager_url", fallback="https://api.tenclass.net/xiaozhi"),
        xiaozhi_manager_secret=parser.get("LLM", "xiaozhi_manager_secret", fallback=""),
