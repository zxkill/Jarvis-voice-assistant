...
def _compose_context() -> str:
    ...
    памяти, чтобы модель учитывала контекст диалога.
    """
    ...
    # Получаем историю общения. Преобразуем каждый элемент в строку,
    # чтобы LLM видела текст, а не структуру данных
    history = list(map(str, short_term.get_last()))

    # Детально логируем сформированный контекст для отладки
    logger.debug(
        "compose_context",
        extra={"ctx": {"history": history, "datetime": timestamp}},
    )

    # Возвращаем историю и метку времени, добавляя штамп последним
