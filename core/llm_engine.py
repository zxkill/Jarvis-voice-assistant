...
def _compose_context() -> str:
    ...
    Первая строка возвращаемого результата содержит человеко‑читаемую
    отметку времени вида ``"Текущая дата 25.04.2024, время 15:30:00"``.
    Эта фраза помогает LLM не путать сегодняшнее число с датами,
    упомянутыми пользователем ранее (например, днём рождения).
    Далее добавляются последние реплики из краткосрочной памяти.
    """
