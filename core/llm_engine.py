...
import re  # работа с регулярными выражениями для очистки истории
...
def _compose_context() -> str:
    """Собрать контекст с явным указанием текущих даты и времени.

    Последняя строка результата содержит человеко‑читаемую отметку вида
    ``"Текущая дата 25.04.2024, время 15:30:00"``.  Размещение штампа в
    конце важно: LLM читает его после всех предыдущих сообщений и тем
    самым воспринимает указанную дату как актуальную для вопросов вида
    «какой сегодня день».  Если же в запросе присутствуют другие даты,
    требующие вычислений (например, «сколько осталось до моего дня
    рождения»), модель должна использовать и их, но не подменять ими
    текущую дату.

    Перед штампом перечисляются последние реплики из краткосрочной
    памяти.  Все встречающиеся в них даты заменяются на заглушку
    ``[дата скрыта]``, чтобы модель не путала прошлые события с
    сегодняшним днём.
    """
    ...
    # Формируем человеко‑читаемую строку даты: «25.04.2024»
    human_day = day.strftime("%d.%m.%Y")
    # Итоговая метка времени с пояснением, как трактовать даты пользователя
    timestamp = (
        f"Текущая дата {human_day}, время {now.isoformat()}. "
    ...
    # Получаем историю общения и переводим её в строки, чтобы LLM работала
    # с текстом. На этом этапе в диалоге могут встречаться сторонние даты,
    # которые способны сбить модель при ответе на вопрос «какой сегодня день».
    raw_history = list(map(str, short_term.get_last()))

    # Регулярные выражения для поиска дат в форматах «7 мая 1988» и
    # «07.05.1988». Такие фрагменты мы заменяем на заглушку
    # ``[дата скрыта]``. Это позволяет сохранить смысл реплик, но исключить
    # подмену текущей даты.
    patterns = [
        re.compile(r"\b\d{1,2}\s+[а-я]+\s+\d{4}\b", re.IGNORECASE),
        re.compile(r"\b\d{1,2}[./-]\d{1,2}[./-]\d{2,4}\b"),
    ]
    # Дополнительный шаблон для поиска поздравлений с днём рождения.
    birthday_re = re.compile(r"с\s+дн[её]м\s+рожд", re.IGNORECASE)
    # Фраза «день рождения» без поздравления. Её также лучше скрывать,
    # чтобы модель не считала, что у пользователя праздник прямо сейчас.
    birthday_plain_re = re.compile(r"день\s+рожд", re.IGNORECASE)

    history: list[str] = []
    for item in raw_history:
        sanitized = item
        for pattern in patterns:
            sanitized = pattern.sub("[дата скрыта]", sanitized)
        # Если в реплике есть упоминание дня рождения, полностью исключаем
        # её из контекста, чтобы модель не продолжала поздравлять пользователя.
        if birthday_re.search(sanitized) or birthday_plain_re.search(sanitized):
            logger.debug("Удалена строка с упоминанием дня рождения: %s", sanitized)
            continue
        history.append(sanitized)

    # Детально логируем исходную и очищенную историю для удобной отладки
    logger.debug(
        "compose_context",
        extra={"ctx": {"history_raw": raw_history, "history": history, "datetime": timestamp}},
    )

    # Возвращаем очищенную историю и метку времени, добавляя штамп последним
