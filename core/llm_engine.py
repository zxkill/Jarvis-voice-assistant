...
def _compose_context() -> str:
    ...
    самым предпочитает настоящую дату любым датам, упомянутым
    пользователем ранее (например, дню рождения).
    ...
    # Итоговая метка времени с подсказкой игнорировать прошлые даты
    timestamp = (
