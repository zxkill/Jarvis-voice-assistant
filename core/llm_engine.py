...
from core.xiaozhi_audio import chat_via_audio
from core.xiaozhi_client import XiaozhiClient, XiaozhiSettings, XiaozhiBindingRequired
...
def _query_xiaozhi(prompt: str, trace_id: str = "") -> str:
    ...
    settings = XiaozhiSettings(
        ...
        client_id=_APP_CFG.llm.xiaozhi_client_id,
        manager_url=_APP_CFG.llm.xiaozhi_manager_url,
        manager_secret=_APP_CFG.llm.xiaozhi_manager_secret,
    ...
    except XiaozhiBindingRequired as exc:
        # Вместо обрыва диалога возвращаем осмысленное сообщение, чтобы
        # пользователь сразу увидел код привязки и ввёл его на сайте.
        logger.warning(
            "Xiaozhi требует привязки устройства", extra={"code": exc.bind_code, "trace_id": trace_id}
        )
        return (
            "Чтобы подключить облачный Xiaozhi, откройте консоль xiaozhi.me, "
            f"выберите привязку устройства и введите код {exc.bind_code}."
        )


def _query_xiaozhi_audio(prompt: str, trace_id: str = "") -> str:
    """Голосовой запрос к Xiaozhi: текст → аудио → Xiaozhi → аудио → текст."""
    ...
    client = XiaozhiClient(settings)
    logger.info(
        "Запускаем аудио-диалог с Xiaozhi", extra={"trace_id": trace_id, "endpoint": settings.endpoint}
    )
    reply = chat_via_audio(client, prompt, trace_id=trace_id)
    _log_llm_exchange(
        stage="xiaozhi-audio",
        prompt=prompt,
        reply=reply,
        trace_id=trace_id,
        context="audio-bridge",
    )
    return reply


def think_xiaozhi_audio(text: str, *, trace_id: str) -> str:
    """Публичный API для голосового общения с Xiaozhi."""

    reply = _query_xiaozhi_audio(text, trace_id=trace_id)
    short_term.add({"trace_id": trace_id, "user": text, "reply": reply})
    long_term.add_daily_event(f"user: {text}\nassistant: {reply}", ["xiaozhi-audio"])
    logger.info("Ответ Xiaozhi (аудио): %s", reply, extra={"trace_id": trace_id})
    return reply
