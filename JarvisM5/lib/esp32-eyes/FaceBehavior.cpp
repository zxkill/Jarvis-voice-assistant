...
eEmotions FaceBehavior::GetRandomEmotion() {
    // Предполагаем, что EMOTIONS_COUNT — последний элемент enum (количество элементов)
    int count = static_cast<int>(eEmotions::EMOTIONS_COUNT);
    int idx = random(0, count);  // [0 .. count-1]
    return static_cast<eEmotions>(idx);
}
...
void FaceBehavior::Update() {
	...
	if (Timer.IsExpired()) {
		...
		if (CurrentEmotion != newEmotion) {
			Logger::log(LogLevel::INFO, "Go to new emotion... %d", newEmotion);
