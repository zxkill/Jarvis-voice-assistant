...
from .sqlite.messages_schema import initialize_messages_schema
...
#
# Стратегия расширения памяти:
#   * сохраняем существующие структуры (``dialog_messages`` и др.), чтобы
#     не ломать обратную совместимость;
#   * специализированные подсхемы (например, универсальная таблица
#     ``messages``) оформляем отдельными модулями в ``memory.sqlite``;
#     миграция подключается из ``_migrate`` и выполняет набор DDL-скриптов.
#   * SQL-комментарии подробно описывают каждое поле, чтобы разработчики
#     быстрее ориентировались при диагностике и построении запросов.
SCHEMA = [
    """
...
def _migrate(conn: sqlite3.Connection) -> None:
    ...
    # После применения базовых миграций подключаем специализированные подсхемы.
    # Сейчас добавлена универсальная таблица ``messages`` с удобными индексами
    # для фильтрации по trace_id, направлению и каналам.
    initialize_messages_schema(conn)
