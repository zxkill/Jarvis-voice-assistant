...
Модуль не управляет сервоприводами — этим занимается :mod:`face_tracker`.
Здесь лишь оценивается вероятность присутствия и при смене состояния
публикуется событие ``presence.update``.
"""
...
# Попытка импортировать OpenCV. В тестовой среде библиотека может отсутствовать
# — тогда модуль загрузится, но детекция по камере будет недоступна. Это
# поведение облегчает запуск тестов и позволяет работать ассистенту на машинах
# без камеры.
...
@dataclass
class PresenceState:
    """Текущее состояние присутствия в кадре."""
...
class PresenceDetector:
    """Оценка присутствия лица и запуск ``FaceTracker``.
    ...
    * при необходимости — автономный цикл обработки кадров с камеры для
    ...
    def __init__(
        ...
        :param threshold: порог уверенности, при превышении которого лицо
            считается найденным
    ...
    def run(self) -> None:
        ...
        Если библиотека OpenCV недоступна либо не задан индекс камеры,
        метод завершается сразу, оставляя систему работоспособной без
