...
Для отладки модуль `PresenceDetector` умеет выводить отдельное окно с
кадром веб‑камеры и рамкой вокруг найденного лица.  Поведение включается
параметром ``show_window`` (по умолчанию активно); закрыть окно можно
клавишей ``q``.  Это помогает убедиться, что камера захватывает изображение
и трекинг работает корректно.
