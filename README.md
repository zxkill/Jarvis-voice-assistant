...
Отладочное окно 800×600 включено по умолчанию (`show_window=true`), но при необходимости его можно отключить в `config.ini`. Ориентация кадра задаётся параметром `frame_rotation` (`0/90/180/270`, по умолчанию `270`) — это полезно, если камера закреплена боком.
...
Для отладки достаточно создать `PresenceDetector` и вызвать метод `run()` — появится окно превью с рамкой вокруг лица (при `show_window=true`). Закрыть окно можно клавишей `q`. При отсутствии OpenCV или при неверном индексе устройства модуль выводит понятные предупреждения и продолжает работу без камеры.

Камера совершает **циклический поворот туда‑сюда** с короткими паузами на краях, что повышает шанс обнаружить человека. Алгоритм использует приращения по горизонтали, поэтому сервопривод быстро меняет направление и не «залипает» на одной стороне. Для корректной работы убедитесь, что подключён серводрайвер M5Stack (`display/drivers/serial.py`) и активирован модуль `presence`.
