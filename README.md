...
- Ведёт контекст текущих даты и времени и добавляет их в каждый запрос к LLM, игнорируя ранее упомянутые пользователем даты (например, день рождения), чтобы поздравления и подсказки были своевременными
