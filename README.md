...
Модуль `core/llm_engine.py` использует Ollama для генерации текста. Клиент
поддерживает профили `light` и `heavy`, что позволяет выбирать между скоростью
и качеством ответа. URL сервера и названия моделей задаются в
`utils/ollama_client.py`.

Краткосрочный контекст диалога хранится в `context/short_term.py`,
а долгосрочные воспоминания — в `context/long_term.py`.
