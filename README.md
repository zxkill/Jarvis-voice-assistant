...
система выводит предупреждение и продолжает работу без распознавания. Для
отладочное окно 800×600 включено по умолчанию (`show_window=true`), но при
необходимости его можно отключить в конфигурации. Если ассистент запускается на
сервере без графического интерфейса, задайте `show_window=false`, чтобы избежать
ошибок отображения.  Ориентация задаётся
параметром `frame_rotation` (`0/90/180/270`, по умолчанию `270`) — это
полезно, если камера закреплена боком.
...
Для отладки достаточно создать ``PresenceDetector`` (окно уже включено) и
вызвать метод ``run()`` — появится превью и рамка вокруг лица.  Закрыть окно
можно клавишей `q`.
При отсутствии OpenCV или при неверном индексе устройства модуль выводит
понятные предупреждения и продолжает работу без камеры. Такой модульный подход
делает трекинг устойчивым и позволяет другим компонентам подписываться на
движения камеры.
