...
Поддерживаются профили `light` и `heavy`, что позволяет выбирать между
скоростью и качеством ответа. HTTP‑вызов выполняется с параметром
`stream=false`, поэтому сервер возвращает единый JSON‑ответ и его легко
парсить. Каждый запрос сопровождается `trace_id`, а реплики пользователя и
модели сохраняются в краткосрочную (`context/short_term.py`) и
долговременную память (`context/long_term.py`).
