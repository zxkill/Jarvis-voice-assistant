...
- Ведёт контекст текущих даты и времени, добавляя штамп в конец каждого запроса к LLM; это заставляет модель игнорировать упомянутые ранее пользователем даты (например, день рождения), чтобы поздравления и подсказки были своевременными
