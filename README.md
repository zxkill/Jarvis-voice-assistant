...
Модуль `sensors/vision/presence.py` при помощи OpenCV и MediaPipe определяет
наличие пользователя в кадре. Уверенность детекции сглаживается методом EMA,
а о смене состояния публикуется событие `presence.update`. При появлении лица
в работу вступает `sensors/vision/face_tracker.py`, который сглаживает
координаты центра, публикует событие `vision.face_tracker` и одновременно
отправляет `track`‑команды драйверу дисплея. Это позволяет сервоприводам
камеры автоматически поворачиваться вслед за пользователем и облегчает
анализ направления взгляда.

Отладочное окно отключается параметром `show_window=false` в `config.ini`.
Перед запуском установите зависимости `opencv-python` и `mediapipe`.
