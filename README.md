...
> Русскоязычный офлайн‑ассистент и эмоциональный робот с горячей перезагрузкой плагинов, поддержкой локальной LLM через Ollama, выводом состояния на OLED через ESP32 по USB‑Serial и расширяемым фреймворком скиллов.
...
* Двумерная модель настроения **Valence–Arousal**, настраиваемая через `config/affect.yaml`
...
* Ожидание ответа на подсказку с публикацией событий `suggestion.response` и
  `dialog.success`/`dialog.failure` (каждое событие содержит `trace_id` для
  сквозной корреляции) и последующим журналированием результата
...
* Надёжный драйвер Serial для M5Stack: NDJSON с меткой времени `ts`, восстановление потерянных скобок и кавычек, строгая валидация входящего потока, фильтрация шумного трафика, игнорирование текстовых логов, потокобезопасная запись с блокировкой и автоматическое восстановление соединения; прошивка отключает вывод логов в USB‑Serial, чтобы канал использовался только для команд
...
* Поведенческое дерево (`behavior tree`) на базе `py_trees` для реактивных действий и микродвижений
* Реалистичные моргания и микродвижения взгляда (микро‑саккады) благодаря логнормальному расписанию и шуму Перлина
* Утилиты генерации случайных чисел и Python rate limiter для контроля частоты событий
* Контекстные приветствия по времени суток через модуль `utils.greeting`
* PID‑регулятор Python с антивиндапом, джерковый профиль движения и IdleScan с синусом/треугольником и шумом
...
├── config/              # YAML-настройки подсистем (affect.yaml, display.yaml, proactive.yaml)
...
├── behavior/            # простое поведенческое дерево (py_trees)
...
В каталоге ``utils`` собраны вспомогательные модули.  ``utils.distributions``
содержит функции ``uniform`` и ``normal`` для генерации случайных
величин.  Класс ``utils.rate_limiter.RateLimiter`` предоставляет простой
Python rate limiter, позволяющий ограничить количество операций за
интервал времени.  Запустить тесты можно так:

```bash
pytest tests/test_utils_distributions.py tests/test_utils_rate_limiter.py
```
...
git clone https://github.com/yourname/Jarvis-voice-assistant.git
...
python3 -m venv venv
# 3. Python-библиотеки
pip install -r requirements.txt  # включает python-dotenv для чтения .env
...
#       (для распознавания лица требуется установленный OpenCV и подключённая веб‑камера)
...
## YAML-файлы в каталоге `config/`

| Файл | Назначение |
|------|------------|
| `affect.yaml` | коэффициенты для расчёта настроения (valence/arousal) |
| `behavior.yaml` | настройка behavior tree и микродвижений |
| `display.yaml` | выбор драйвера дисплея и последовательного порта |
| `proactive.yaml` | включение и тайминги системы проактивных подсказок |

Отредактируйте YAML-файлы в каталоге `config/` (при отсутствии используются значения по умолчанию):
- `affect.yaml` — коэффициенты настроения
- `behavior.yaml` — структура behavior tree и параметры морганий
- `display.yaml` — параметры драйвера дисплея
- `proactive.yaml` — интервалы и каналы проактивных подсказок

### Настройка `affect.yaml`

Файл `config/affect.yaml` задаёт влияние событий на координаты **Valence–Arousal**.
Пример:
...
### Настройка `behavior.yaml`

Файл `config/behavior.yaml` описывает структуру `behavior tree` и параметры микродвижений (например, интервал моргания).
Пример:

```yaml
root: [greet, blink, idle]
blink:
  interval_ms: 3000
```

После изменения YAML‑файлов перезапустите ассистента.
...
Jarvis разделяет обнаружение человека и реальный **face tracking**:

Детектор построен на OpenCV (каскад Хаара); если библиотека не установлена,
система выводит предупреждение в лог и продолжает работу без распознавания.


* `sensors/vision/presence.py` фиксирует появление лица в кадре и публикует событие `presence.update`
* `sensors/vision/face_tracker.py` сглаживает координаты, поворачивает камеру и рассылает событие `face_tracker.update`

Для отладки модуль `PresenceDetector` умеет выводить отдельное окно
размером 800×600 с кадром веб‑камеры и рамкой вокруг найденного лица.
Параметр ``show_window`` отвечает за его отображение (по умолчанию
активно), а ``frame_rotation`` (значение по умолчанию 270°) позволяет
повернуть кадр на 90° / 180° / 270° при нестандартной установке камеры.
Закрыть окно можно клавишей ``q``.  Это помогает убедиться, что камера
захватывает изображение и трекинг работает корректно.

Чтобы уменьшить ложные срабатывания (например, когда **велосипед** принимается за лицо),
добавлены настройки каскадного детектора:

* ``scale_factor`` — шаг пирамиды масштабов; значения > 1.0, по умолчанию 1.1;
* ``min_neighbors`` — требуемое количество совпадающих окон; повышайте для фильтрации шума;
* ``min_size`` — минимальный размер лица в пикселях;
* ``require_eyes`` — дополнительно проверять наличие глаз внутри прямоугольника.

Корректная настройка этих параметров улучшает распознавание лица и позволяет избежать
ошибок с посторонними объектами. Все параметры сопровождаются подробным журналированием,
что облегчает диагностику и повышает точность **распознавания лица** на базе OpenCV.

Такой модульный подход делает трекинг устойчивым и позволяет другим компонентам подписываться на движения камеры.
...
4. Поле `global_rate_limit_ms` ограничивает общую частоту срабатываний,
   чтобы Jarvis не «заспамливал» эффекты при бурных событиях.
5. Политика настроения передаёт текущую `sfx_palette`, и манифест может
   содержать варианты эффектов вида `BRIGHT:ACK` для разных оттенков
   настроения.
...
нескольких секунд и публикует события `suggestion.response` и
`dialog.success`/`dialog.failure` с тем же `trace_id`, что помогает анализировать
реакцию пользователя и строить сквозную корреляцию в логах. На основе собранных
...
Для проверки работоспособности и измерения покрытия кода выполните:

```bash
pytest --cov
```

Тесты имитируют случайность и время, поэтому результаты стабильны и
позволяют достичь близкого к 100% покрытия для ключевых модулей.
...
В директории `behavior/` находится демонстрационное **поведенческое дерево** на
библиотеке `py_trees`. Оно управляет реакциями ассистента: приветствует при
обнаружении лица, моргает по запросу и переходит в режим ожидания, когда условий
нет. Чтобы увидеть дерево в действии, выполните:

```bash
pytest tests/test_behavior_tree.py
```

Ключевые слова: *behavior tree*, *py_trees*, *AI поведение*.

## Контроль движений и IdleScan

Модули `control.pid`, `control.motion_profile` и `control.scan_patterns` помогают управлять сервоприводами и камерами. PID‑регулятор с антивиндапом и ограничением скорости выдаёт плавное управление, профиль движения ограничивает ускорение и джерк, а IdleScan генерирует синусоидальные и треугольные траектории с шумом.

```python
from control.pid import PID
pid = PID(1.0, 0.1, 0.01, max_output_rate=0.5)
control = pid.update(setpoint, measurement, dt)
```

```python
from control.scan_patterns import idle_scan
points = idle_scan('sine', 100, noise_std=0.05)
```

Ключевые слова: PID регулятор Python, ограничение ускорения, джерк, IdleScan, синусоидальный паттерн, треугольная волна.
