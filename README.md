...
Для отладки достаточно запустить метод ``run()`` у ``PresenceDetector`` — в
логе появится информация о запуске камеры и фиксации лица. При отсутствии
OpenCV или при неверном индексе устройства модуль выводит понятные
предупреждения и продолжает работу без камеры. Такой модульный подход делает
трекинг устойчивым и позволяет другим компонентам подписываться на движения
камеры.
