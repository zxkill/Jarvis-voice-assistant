...
- Календарный скилл сообщает текущую и завтрашнюю дату
 - Ведёт контекст текущих даты и времени, добавляя штамп в конец каждого запроса к LLM; вопросы о сегодняшнем дне опираются на этот штамп, но для расчётов с другими датами (например, до дня рождения) используется дата из запроса
 - Перед обращением к LLM все упомянутые в истории диалога даты маскируются, чтобы исключить подмену текущего дня
 - Упоминания дня рождения очищаются из истории, поэтому ассистент не будет поздравлять без прямого запроса
