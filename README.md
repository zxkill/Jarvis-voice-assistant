...
отладки можно включить окно превью (`show_window=True`) и задать поворот кадра
в градусах (`frame_rotation` = `0/90/180/270`) — это полезно, если камера
установлена боком.
...
Для отладки достаточно создать ``PresenceDetector`` c ``show_window=True`` и
вызвать метод ``run()`` — появится окно 800×600 с кадром и рамкой вокруг лица.
