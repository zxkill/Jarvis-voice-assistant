...
11. [Управление Jarvis через Telegram](#управление-jarvis-через-telegram)
12. [Прошивка для M5Stack Core ESP32](#прошивка-для-m5stack-core-esp32)
13. [Разработка своих скиллов](#разработка-своих-скиллов)
14. [Roadmap и лицензия](#roadmap-и-лицензия)
...
Когда слушатель активен и подключён владелец, каждое голосовое
уведомление дублируется текстом в личные сообщения Telegram.
Факт отправки фиксируется метрикой `telegram.outgoing`, а сетевые ошибки
логируются как предупреждения, не прерывая озвучивание. Если команда
пришла через Telegram‑бота, ответ отправляется туда же текстом без
озвучивания на устройстве. В Telegram слово активации не требуется —
можно сразу писать нужную команду.
...
Для проверки корректности установки и кода предусмотрены unit-тесты на
основе `pytest`. Они покрывают ключевые подсистемы, включая приём команд
через Telegram и обработку сетевых ошибок. Запустите все тесты командой:
...
3. Каждый эффект может задать паузу `cooldown_ms`; функция `play_effect`
   учитывает этот интервал и не даёт одному и тому же звуку
   воспроизводиться чаще, что особенно полезно для предотвращения
   многократного повторения звука вздоха.
...
поэтому типичные проблемы вроде "звук вздоха повторяется пять раз подряд"
теперь исключены. Это улучшает восприятие и помогает по запросам «повтор
звука вздоха Jarvis» быстрее найти нужное решение.
...
* Утром Jarvis приветствует пользователя бодрой фразой.
