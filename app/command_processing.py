...
async def _delegate_to_llm(text: str, *, channel: str, trace_id: str) -> str:
    ...
    try:
        # Для Telegram и облачного Xiaozhi используем аудио-мост, чтобы сервер
        # получил голосовой запрос и вернул звук. Для остальных сценариев
        # сохраняем текстовый путь.
        if llm_engine.LLM_BACKEND == "xiaozhi" and channel == "telegram":
            reply = await asyncio.to_thread(
                llm_engine.think_xiaozhi_audio, text, trace_id=trace_id
            )
        else:
